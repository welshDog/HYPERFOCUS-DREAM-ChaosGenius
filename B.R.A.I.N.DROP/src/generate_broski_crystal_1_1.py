        ...
        try:
            ...
            query = f"""
                SELECT id, content, timestamp, memory_type, context, importance,
                       associations, energy_level, tags FROM memories
            ...
            for row in cursor.fetchall():
                memory = {
                    ...
                    "importance_score": row[5],  # Map importance to importance_score
                    "associations": json.loads(row[6] or "[]"),
                    "energy_level": row[7],
                    "context_tags": json.loads(row[8] or "[]"),
                    "emotional_weight": row[
                        5
                    ],  # Use importance as emotional weight for now
                    "crew_member": "BROski_X",  # Default crew member since not in current schema
                    "retrieval_count": 0,  # Default retrieval count
                }
